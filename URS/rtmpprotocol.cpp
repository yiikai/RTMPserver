...
int rtmpprotocol::distinguishpacket(rtmpcommonmessage* msg, int headtype, int streamid)
{
	...
	else if (command_name == "FCUnpublish")
	{
		...
		int ret = packet->decodepayload(msg);
		delete packet;
		//½ÓÊÜµ½unpublishµÄcommandÖ®ºó¾Í¿ÉÒÔ½øÐÐfdµÄ¹Ø±Õ²Ù×÷ÁË
	...
	else if (command_name == "createStream")
	{
		...
		int ret = packet->decodepayload(msg);
		rtmpcreatestreamrespacket* res_packet = new rtmpcreatestreamrespacket(packet->transaction_id, 1);    //srsÒ²ÊÇÖ¸¶¨Ä¬ÈÏ»Ø¸´createstreamÓÃµÄÊÇstreamid = 1
	...
	}else if (command_name == "publish")
	{
		...
		//¼ÇÂ¼µÄrtmpµØÖ·ÐèÒªÆ´½ÓÉÏ²¥·ÅÂ·¾¶:Àý×Ó:rtmp://127.0.0.1/live/123, 123¾ÍÊÇ²¥·ÅÂ·¾¶
	...
	else if (command_name == "play")
	{
		...
		int ret = packet->decodepayload(msg);
		//the same as publish , ÐèÒª½øÐÐreqµÄurlµÄÆ´½Ó¹¤×÷£¬ÓÃÆ´½ÓºóµÄurlÈ¥»ñÈ¡steamsource
		req.tcUrl += packet->stream_name;
		...
		streamsource* ss = streamsource::getInstance();
		m_source = ss->findsource(&req);
		//¶ÔÓÚplayµÄ¿Í»§¶Ë¶øÑÔÈç¹ûÃ»ÓÐµÃµ½streamsource¾ÍËµÃ÷Ã»ÓÐÍÆÁ÷¶Ë´æÔÚ£¬²»Ó¦¸Ã¸ø¿Í»§¶Ë´«µÝÈÎºÎstream
		if (!m_source)
		{
...
int rtmpprotocol::send_and_free_message_to_client(sharedMessage* msg, int clientfd)
{
	...
	while (p < pend)
	{
		...
		if (p == msg->messagepalyload)
		{
			...
			if (totalsize > 4096)
			{
				sendbuf = new char[4096 + hn];    //ÕâÀïÓÃchunkheaderµÄ´óÐ¡¼ÓÉÏÐ­ÉÌµÄchunksizeµÄ´óÐ¡Ô¸ÒâÊÇffmpegÔÚ
				//½âÎöµÄÊ±ºò µ±´óÓÚchunksizeµÄ°ü»á°´ÕÕchunksize¶ÁÈ¡Êý¾Ý£¬ÕâÑù¾Íµ¼ÖÂÊµÌå·¢ËÍµÄÊý¾ÝÃ»ÓÐchunksizeµÄ´óÐ¡
				totalsize = 4096 + hn;
			}
			else
			{
...
int rtmpprotocol::pull_and_send_stream()
{
	...
	//Ä¿Ç°¶ÔÓÚÀ­Á÷Ã»ÓÐ²ßÂÔ£¬À­Ò»¸ö³ö¶ÓÁÐÒ»¸ö
	m_source->pop_msg();
	//ÍÆËÍÁ÷Êý¾Ýµ½¿Í»§¶Ë
	if (-1 == send_and_free_message_to_client(msg))
	{
