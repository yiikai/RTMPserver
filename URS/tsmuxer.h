...
#define TS_PMT_NUMBER 1    //Ä¿Ç°Ö»Ö§³ÖÒ»¸öpmt±í
#define TS_PMT_PID 0x1001
...
public:
	...
	*/
	...
	int PES_header_data_length;   //´ÓSRSÔ´Âë¿´Ó¦¸ÃÊÇPES_header_data_length×Ö¶ÎÖ®ºóµÄÌî³äÊý¾ÝµÄ´óÐ¡
...
public:
    ...
    int open_muxfile();
    int init_muxfile();
    int recv_video_data(char* data, int len);
    int pop_tsqueue(avcsample &sample);
private:
...
public:
    queue<avcsample> m_tsqueue;
    pthread_mutex_t m_tsmux_mutex;
    bool m_isfirst;
    bool m_stratwrite;   //fixme: bug when write ts file first frame not key frame need wait for first key frame
